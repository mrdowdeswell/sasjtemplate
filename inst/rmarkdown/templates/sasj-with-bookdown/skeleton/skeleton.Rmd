...
The default `output` format in the YAML frontmatter of this template is `sasjtemplate::with_bookdown`.
...
This makes use of the additional citing and referencing functionality in the `bookdown` package^[The `bookdown` package, and its dependencies, are installed when the `sasjtemplate` package is installed.] [@yihui2016].

The `bookdown` package allows some finer control over certain features, as indicated in subsequent sections.
...
Alternatively, you may reference equations (and sections, subsections, tables, figures, etc.)\ using the syntax `\@ref(eqn:Poisson1)` instead of (`\ref{eqn:Poisson1}`), the latter being essentially raw \LaTeX. As mentioned before, this is a matter of preference.
...
You may write your theorems, propositions, etc.\ as in Section \ref{sec:thms} (using regular \LaTeX), and use the referencing syntax described in Subsection \ref{subsec:mathexpress}.

### Using the `sasjtemplate::with_bookdown` output format functionality

The `sasjtemplate` exposes functionality from the `bookdown` package. It is therefore also possible to use the following markdown syntax to provide a theorem (and a label for it, `#thmlabel`, to use for referencing).
...
::: {.theorem #thmlabel}
The advantage of this `bookdown` theorem environment syntax is that it may contain **any** _Markdown_ syntax^[Even footnotes!].
:::
