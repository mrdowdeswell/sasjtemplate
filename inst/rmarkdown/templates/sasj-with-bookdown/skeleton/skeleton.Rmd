...
## Template YAML - LEAVE AS IS UNLESS YOU KNOW WHAT YOU'RE DOING
output: 
  sasjtemplate::with_bookdown:
#  pdf_document:
#  bookdown::pdf_document2:
#    template: SASJ-Rmd-Template.tex
...
We proceed to recast some of the earlier \LaTeX-produced sections using R markdown syntax. In principle, it is possible to write a complete SASJ-submission-ready document entirely within R markdown (using this template) without ever touching a \LaTeX\ compiler directly^[Of course, behind the `rmarkdown` and `knitr` scenes, Pandoc is calling a \LaTeX\ compiler on your behalf. To this end, it is recommended that you install the `tinytex` R package, a lightweight \LaTeX\ distribution.]. 

## Template file structure {#subsec:filestructure}

Before starting with the details of the template files themselves, you should be aware that your `.Rmd` file (well, Pandoc, really) will need to be able to locate four files used in the creation of the SASJ article: `SASJ_Rmd_Template.tex`, `SASJ.sty`, `sasj.bst`, `references.bib`. The first of these is the Pandoc template that is used to render the article. The last three files are the original files provided as part of the SASJ TeX template [see, for example, @SASJtextemplate]. Of the four, the only one you would need to edit or replace is `references.bib` (with your own references relevant to your article).

If your `.Rmd` document (your edited version of a copy of the `Rmd for SASJ.Rmd` template) is in the same folder as these files, then you should have no problem compiling to `.pdf` (you should try it). (You should rename the `Rmd for SASJ.Rmd` file before using it---for example, `Inference article for SASJ.Rmd`.)

Alternatively, it is possible to install the R markdown template for SASJ into your version of R Studio so that it is one of the available templates when clicking through the menu items `File > New File > R Markdown...`, and then navigate to `From Template` in the left pane.
...
The default `output` format in the YAML frontmatter of this template is `pdf_document`.
...
Alternatively, for users familiar with the `bookdown` package^[Even if you are not familiar with it, it is easy to install via `install.packages("bookdown")` in R.] [@yihui2016], it is also possible to use `bookdown::pdf_document2` as the `output` format.
...
Which `output` format to use is not a huge decision. It is largely a matter of familiarity. The `bookdown` package allows some finer control over certain features, as indicated in subsequent sections.
...
If you choose to use `bookdown::pdf_document2` as your `output` format, then you may alternatively reference equations (and sections, subsections, tables, figures, etc.)\ using the syntax `\@ref(eqn:Poisson1)` instead of (`\ref{eqn:Poisson1}`), the latter being essentially raw \LaTeX. As mentioned before, this is a matter of preference.
...
### Using vanilla `pdf_document` output format

If you use the `pdf_document` output format, then you may write your theorems, propositions, etc.\ as in Section \ref{sec:thms} (using regular \LaTeX), and use the referencing syntax described in Subsection \ref{subsec:mathexpress}.

### Using the `bookdown` package and `bookdown::pdf_document2` output format

If you choose to optionally use the `bookdown::pdf_document2` output format, then it is also possible to use the following markdown syntax to provide a theorem (and a label for it, `#thmlabel`, to use for referencing).

    ::: {.theorem #thmlabel}
    The advantage of this `bookdown` theorem environment syntax is that it may contain 
    **any** _Markdown_ syntax^[Even footnotes!].
    :::

```{r show-theorem-if-bookdown-being-used, results = 'asis'}
if (
  "bookdown::pdf_document2" == rmarkdown::all_output_formats(knitr::current_input())   |
  "bookdown::pdf_book" == rmarkdown::all_output_formats(knitr::current_input())
  |
  "sasjtemplate::with_bookdown" == rmarkdown::all_output_formats(knitr::current_input())
  ) {
cat('::: {.theorem #thmlabel}', 'The advantage of this `bookdown` theorem environment syntax is that it may contain **any** _Markdown_ syntax^[Even footnotes!].', ':::', sep = '\n')
}
```
...
The SASJ guidelines don't address appendices at all. If, however, you wish to try your luck, you will _have_ to use `bookdown::pdf_document2` as the `output` format for the `.Rmd` file. Then, add appendices using:
